...
import com.example.emtask.presentation.ui.components.diffUtil.VacanciesDiffCallback
...
internal class VacanciesAdapter(private val vacationClickListener: VacationClickListener) :
    ...
    var vacancies: List<VacancyEntity> = listOf()
        set(value) {
            val result = DiffUtil.calculateDiff(VacanciesDiffCallback(field, value))
            field = value
        ...
        fun bind(vacancy: VacancyEntity, position: Int) {
            ...
            isFavourite.setOnClickListener {
                vacationClickListener.onClickUpdateVacancy(
                    vacancyEntity = vacancies[position].copy(isFavorite = !vacancy.isFavorite),
                )
            }
        }
    }
